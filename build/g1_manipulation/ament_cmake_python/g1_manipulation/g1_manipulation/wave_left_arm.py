...
class JointStatePublisher(Node):
    ...
    def __init__(self):
        ...
        super().__init__('my_left_arm_joint_state_publisher')
    ...
    def publish_joint_states(self):
        ...
        # Apply coordinated motion to left arm joints
        # Each joint gets different motion for natural waving
        
        # Index 13: Shoulder pitch - slight up/down motion
        msg.position[13] = 0.2 * math.sin(wave_frequency * self.time_elapsed)
        
        # Index 14: Shoulder roll - main waving motion (most important!)
        # 0.8 rad offset raises arm, sine wave adds waving motion
        msg.position[14] = 0.8 + 0.3 * math.sin(wave_frequency * self.time_elapsed)
        
        # Index 15: Shoulder yaw - rotation motion
        msg.position[15] = wave_position
        
        # Index 16: Elbow - faster motion (frequency × 2) with offset
        # 0.3 rad keeps elbow slightly bent, adds secondary motion
        msg.position[16] = 0.3 + 0.2 * math.sin(wave_frequency * self.time_elapsed * 2)
        
        # Wrist joints - smaller motions that follow the main wave
        msg.position[17] = wave_position * 0.5    # Wrist roll (50% of main motion)
        msg.position[18] = wave_position * 0.3    # Wrist pitch (30% of main motion)  
        msg.position[19] = wave_position * 0.4    # Wrist yaw (40% of main motion)
